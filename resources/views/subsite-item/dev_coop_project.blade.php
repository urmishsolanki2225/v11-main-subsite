    ...
    <section class="dev_coop_project_benefitting_countries dcprojects-meta">
        <dl>
            <dt>
                ...
                <dd><a href='{{ route("subsite.resources.coop_projects.index",["filter" => ["country" => $country->id]]) }}'>
            ...
            @foreach ($partners['grouped'] as $countryName => $partners)
                @if ($countryName === '_' || $countryName === '')
                    @foreach ($partners as $partner)
                        ...
                        <ul>
                            <li>
                                <a href='{{ route("subsite.resources.coop_projects.index", ["filter" => ["partner" => $partner->name ]]) }}' class="non_member_partner">
                                    {{ $partner->name }}
                                    @if ($partner->acronym)
                ...
                @else
                    ...
                    <ul>
                        @foreach ($partners as $partner)
                            <li>
                                <a href='{{ route("subsite.resources.coop_projects.index", ["filter" => ["partner" => $partner->name ]]) }}' @class([
                ...
                <dd>
                    <ul>
                        @foreach ($taxonomySub->subCollections as $collection)
                            <li>
                                <a href='{{ route("subsite.resources.coop_projects.index",["filter" => ["taxonomy" => $collection->id]]) }}'>
