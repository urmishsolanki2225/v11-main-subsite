...
@section('collection_header')
    ...
    @foreach ($collection->parentCollectionsOfType('region') as $region)
        <x-link-subsite :collection="$region" class="link_back" />
    @endforeach

@endsection

@section('collection_subnavigation')
    @include('collection.subsite-country_subnavigation', ['activeSlotId' => false])
@endsection
        ...
        @isset ($groupedItems['article'])
            <section class="collection_content articles-listing">
                ...
                @for ($i = 0; $i < min(6, count($groupedItems['article'])); ++$i)
                    <li>
                    @include('shared.subsite-card', ['item' => $groupedItems['article'][$i], 'header_level' => '4', 'show_blurb' => true])
                    </li>
                @endfor
                </ol>
                <a href="{{ route('subsite.country.articles', ['id' => $collection->id, 'slug' => $collection->content->slug]) }}" class="archive_link">{{ __('eiie.All articles')}}: {{ $collection->content->title }}</a>
            </section>
        @endisset
        ...
        @isset ($groupedItems['static'])
            <section class="collection_content generic-items-listing">
                ...
                @for ($i = 0; $i < min(4, count($groupedItems['static'])); ++$i)
                    <li>
                    @include('shared.susbite-card', ['item' => $groupedItems['static'][$i], 'header_level' => '4'])
                    </li>
                @endfor
                </ol>
            ...
            <section class="affiliates-listing">
                ...
                <ol class="collection_affiliates">
                    @foreach ($affiliates as $item)
                        <li>@include('shared.subsite-card', ['header_level' => '4'])</li>
                    @endforeach
                </ol>
            </section>
        @endisset
