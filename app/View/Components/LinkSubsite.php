...
class LinkSubsite extends Component
{
    public $class;
    public $href;
    public $title;
    ...
    public function __construct(
        ...
        } elseif (isset($subsite)) {
            $this->title = $subsite->name;
            // Subsite section end
        } else {
            abort(500);
        }
        ...
        if (isset($subsite)) {
            ...
            $this->href = $get_url['scheme'] . '://' . $subsite->aliase_name . '.' . $get_url['host'] . '/' . $lang;
            $this->target = "_blank";
        } else {
            $routeName = "subsite." . $routeName;
            $this->title = $title ? $title : $_title;
