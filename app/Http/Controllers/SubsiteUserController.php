...
class SubsiteUserController extends Controller
{
    ...
    {
        $campaigns = config('eiie.item.home-subsite');
        $campaignValue = $campaigns[$this->subsites['subsites']->aliase_name];
        $homeItem = Item::withoutGlobalScopes()->find($campaignValue);

        $news_id = config("eiie.collection.news");

        $featsubsitearray = config('eiie.collection.featured-subsite');
        $featsubsiteid = $featsubsitearray[$this->subsites['subsites']->aliase_name];
